import type { SocialLink } from "@/types/content";

export const socialLinks: SocialLink[] = [
  {
    label: "Twitch",
    handle: "nari_nuna",
    url: "https://www.twitch.tv/nari_nuna",
    purpose: "Live streams and the heart of the on-air Haven",
    verifiedAt: "2026-08-13",
    external: true
  },
  {
    label: "YouTube",
    handle: "@Nari_Nuna",
    url: "https://www.youtube.com/@Nari_Nuna",
    purpose: "Curated videos, shorts, nail content, and highlights",
    verifiedAt: "2026-08-13",
    external: true
  },
  {
    label: "VOD Archive",
    handle: "@NariNunaVod",
    url: "https://www.youtube.com/@NariNunaVod",
    purpose: "Long-form stream archives, including adult-marked entries",
    verifiedAt: "2026-08-13",
    external: true
  },
  {
    label: "X",
    handle: "@Nari_Nuna",
    url: "https://x.com/Nari_Nuna",
    purpose: "Updates, art credits, and creator conversation",
    verifiedAt: "2026-08-13",
    external: true
  },
  {
    label: "TikTok",
    handle: "@narinuna",
    url: "https://www.tiktok.com/@narinuna",
    purpose: "Short-form stream chaos and clips",
    verifiedAt: "2026-08-13",
    external: true
  },
  {
    label: "Instagram",
    handle: "@narinunagotu",
    url: "https://www.instagram.com/narinunagotu",
    purpose: "Gaming moments, nail-polish life, and visual updates",
    verifiedAt: "2026-08-13",
    external: true
  },
  {
    label: "Linktree",
    handle: "NariNuna",
    url: "https://linktr.ee/NariNuna",
    purpose: "Nari's current public link hub",
    verifiedAt: "2026-08-13",
    external: true
  }
];

export const twitchUrl = "https://www.twitch.tv/nari_nuna";
export const discordUrl = "https://discord.com/invite/f25YtvtnbV";
export const throneUrl = "https://throne.com/narinuna";
