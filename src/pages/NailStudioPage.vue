...
const studioPieces: { icon: typeof Images; title: string; text: string; ghostie: GhostieVariant }[] = [
  { icon: Images, title: "Her real work", text: "Nari's own sets and photographs belong here when she chooses which ones to share.", ghostie: "portfolio" },
  { icon: Paintbrush, title: "The fun part", text: "Color stories, tiny details, glitter experiments, and the occasional wonderful accident.", ghostie: "play" },
  ...
  <section class="studio-notes page-width section-pad">
    ...
    <div class="studio-notes__grid studio-notes__grid--painted">
      <article v-for="piece in studioPieces" :key="piece.title">
        <GhostieIllustration class="ghostie-card-art" :variant="piece.ghostie" />
...
<style scoped>
.studio-notes__grid--painted { align-items: stretch; }
.studio-notes__grid--painted > article { grid-template-rows: 12rem 1fr; min-height: 100%; }
.ghostie-card-art { height: 12rem; border-bottom: 1px solid var(--story-line); border-radius: 0; }
.studio-notes__grid--painted > article > div { min-height: 13.75rem; }
.studio-gallery-note { margin-block-start: clamp(2rem, 7vw, 3.5rem); }
@media (min-width: 48rem) {
  .studio-notes__grid--painted > article { grid-template-rows: 13rem 1fr; }
  .ghostie-card-art { height: 13rem; }
  .studio-notes__grid--painted > article > div { min-height: 14.5rem; }
}
</style>
